...
class DocumentAnalysisParser(Parser):
            ...
            for page in analyze_result.pages:
                ...
                if analyze_result.tables:
                    for table in analyze_result.tables:
                        # Check if any cell in the table belongs to this page
                        if any(
                            cell.bounding_regions and cell.bounding_regions[0].page_number == page.page_number
                ...
                for idx, mask_char in enumerate(mask_chars):
                    ...
                    if object_type == ObjectType.NONE:
                        ...
                        if mask_char not in added_objects:
                            target_table = tables_on_page[object_idx]
                            title = DocumentAnalysisParser.get_table_title(
                                target_table, analyze_result.paragraphs or [], page.page_number
                            )
                            table_html = DocumentAnalysisParser.table_to_html(
                                target_table, current_page_num=page.page_number, title=title
                            )
    ...
    @staticmethod
    def get_table_title(table: DocumentTable, paragraphs: list[DocumentParagraph], page_num: int) -> str:
        if not table.bounding_regions:
            return ""

        table_top_y = table.bounding_regions[0].polygon[1]  # y0 coordinate

        candidates = []
        for p in paragraphs:
            if not p.bounding_regions or p.bounding_regions[0].page_number != page_num:
                continue

            para_bottom_y = p.bounding_regions[0].polygon[5]  # y1 coordinate

            # Check if paragraph is close above the table (e.g., within 0.5 inch / 36-50 pixels)
            if 0 < (table_top_y - para_bottom_y) < 50:
                candidates.append(p)

        if candidates:
            # Sort by proximity (largest Y at bottom is closest to table top)
            closest = sorted(candidates, key=lambda p: p.bounding_regions[0].polygon[5], reverse=True)[0]
            return closest.content

        return ""

    @staticmethod
    def table_to_html(table: DocumentTable, current_page_num: int | None = None, title: str = ""):
        table_html = "<figure>"
        if title:
            table_html += f"<figcaption>{html.escape(title)}</figcaption>"
        table_html += "<table>"
        relevant_cells = []
        for cell in table.cells:
            is_header = cell.kind == "columnHeader" or cell.kind == "rowHeader"
            if (
                not current_page_num
                or is_header
                or (cell.bounding_regions and cell.bounding_regions[0].page_number == current_page_num)
            ):
                relevant_cells.append(cell)

        rows_map = {}
        for cell in relevant_cells:
            if cell.row_index not in rows_map:
                rows_map[cell.row_index] = []
            rows_map[cell.row_index].append(cell)
